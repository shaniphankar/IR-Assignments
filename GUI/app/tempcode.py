...
def score_query(query_tokens,tf_idf_store,reviews):
	...
	for document,content in tf_idf_store.items():
		...
		if len(queue)<41:
			heapq.heappush(queue,(score,document,reviews[document]['name']))
...
def main():
	query = input()
	dict = {}
	#create the dictionary which maps business ids to their names
	with(open(os.getcwd()+'/yelp_academic_dataset_business.json')) as f:
		objects =(json.loads(line) for line in f)
		for obj in objects:
			dict[obj['business_id']]=obj['name']
	#
	with(open(os.getcwd()+'/yelp_academic_dataset_review.json')) as f:
		...
		for x in objects:
			...
			reviews[x['review_id']]={'business':x['business_id'],'name':dict['business_id'],'numWords':numWords,'stars':x['stars'],'text':tokens}
